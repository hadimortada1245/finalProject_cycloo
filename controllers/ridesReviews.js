...
const add = async (req, res) => {
    try {
        ...
        if (!description || !rate || !user_id || !ride_id) throw Error("All fields must be filled");
        const addQuery = `INSERT INTO ridesReviews (
            user_id,
            ride_id,
            description,
            rate
        ...
        const result = await con.promise().query(addQuery, [user_id, ride_id, description, rate]);
...
const getReviews = async (req, res) => {
    try {
        ...
        const getQuery = `SELECT * FROM ridesReviews WHERE user_id = ? `;
